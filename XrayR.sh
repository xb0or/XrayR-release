...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/xb0or/XrayR-release/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/xb0or/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
